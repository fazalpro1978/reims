...
async function uploadFile(
  file: File,
  slot: 'photo' | 'certificate',
  token: string,
): Promise<{ url: string; fileType: 'image' | 'pdf' }> {
  const fd = new FormData();
  fd.append('file', file);
  fd.append('slot', slot);
  const res = await fetch('/api/dashboard/circle-of-excellence/upload', {
    method: 'POST',
...
export default function CircleOfExcellence() {
  ...
  const openEdit = () => {
    setForm(spotlight ? spotlightToForm(spotlight) : EMPTY_FORM);
    setPhotoPreview(spotlight?.photo_url ?? '');
    setSaveErr('');
    setEditing(true);
  };
  ...
  const handlePhotoFile = async (file: File) => {
    const token = (await (await fetch('/api/auth/token')).json().catch(() => null))?.token;
    // Get token from authedFetch headers approach
    setPhotoLoad(true);
    try {
      const res = await authedFetch('/api/dashboard/circle-of-excellence/upload', {
        method: 'POST',
        body: (() => { const fd = new FormData(); fd.append('file', file); fd.append('slot', 'photo'); return fd; })(),
      });
      const j = await res.json();
  ...
  const handleCertFile = async (file: File) => {
    ...
    try {
      const res = await authedFetch('/api/dashboard/circle-of-excellence/upload', {
        method: 'POST',
        body: (() => { const fd = new FormData(); fd.append('file', file); fd.append('slot', 'certificate'); return fd; })(),
      });
      const j = await res.json();
      if (!res.ok) { setSaveErr(j.error ?? 'Certificate upload failed'); return; }
      setForm(f => ({ ...f, certificate_url: j.url, certificate_type: j.fileType }));
    ...
    return (
      <div style={{ ...cardStyle, borderColor: 'rgba(201,168,76,.35)', boxShadow: '0 0 0 1px rgba(201,168,76,.12)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...ghostBtn,
                background: saving ? 'rgba(201,168,76,.1)' : '#c9a84c',
                color: saving ? '#c9a84c' : '#0f0f0f',
                fontWeight: 700,
                borderColor: '#c9a84c',
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
          ...
          />
          <UploadZone
            label="Certificate (PDF / PNG / JPG / WebP)"
            accept="image/png,image/jpeg,image/webp,application/pdf"
            preview={form.certificate_type === 'image' ? form.certificate_url : ''}
            loading={certLoad}
          ...
          {form.certificate_url && form.certificate_type === 'pdf' && (
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ height: 100, border: '1.5px solid #2a2a3e', borderRadius: 8, overflow: 'hidden', background: '#0d0d14' }}>
                <iframe src={form.certificate_url} style={{ width: '100%', height: '100%', border: 'none' }} title="Certificate" />
              </div>
            </div>
          )}
        </div>
        ...
        <label style={labelWrap}>
          ...
          <textarea
            style={{ ...inputStyle, height: 88, resize: 'vertical', lineHeight: 1.6 }}
            placeholder="Write a custom announcement, praise, or recognition message…"
            value={form.message}
        ...
        {saveErr && (
          <p style={{ margin: '10px 0 0', fontSize: 11, color: '#ef4444' }}>{saveErr}</p>
        )}
      </div>
    );
  }

  // ── No spotlight yet (admin sees prompt, others see nothing) ────────────────

  if (!spotlight || !spotlight.employee_name) {
    ...
    return (
      <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px' }}>
        ...
        <button onClick={openEdit} style={{ ...ghostBtn, borderColor: '#c9a84c', color: '#c9a84c' }}>
          + Add Spotlight
        </button>
      </div>
    );
  ...
  const hasCert = !!(spotlight.certificate_url);
  ...
  return (
    <>
      ...
      {certOpen && hasCert && (
        ...
        >
          <div
            ...
            style={{
              width: 'min(860px, 90vw)', height: 'min(640px, 85vh)',
          ...
          >
            <button
              ...
              style={{
                ...
                background: 'rgba(0,0,0,.6)', border: 'none', borderRadius: 6,
                color: '#888', fontSize: 13, cursor: 'pointer', padding: '4px 8px',
              }}
            >✕</button>
            {spotlight.certificate_type === 'pdf' ? (
              <iframe
                src={spotlight.certificate_url!}
      ...
      <div style={{
        ...cardStyle,
        borderImage: 'linear-gradient(135deg, rgba(201,168,76,.3) 0%, rgba(201,168,76,.05) 60%, transparent 100%) 1',
        position: 'relative', overflow: 'hidden',
      }}>
        {/* Subtle gold shimmer bar at top */}
        <div style={{
          position: 'absolute', top: 0, left: 0, right: 0, height: 2,
          background: 'linear-gradient(90deg, transparent, #c9a84c, #dfc070, #c9a84c, transparent)',
          opacity: .65,
        }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          ...
          {isAdmin && (
            <button onClick={openEdit} style={{ ...ghostBtn, fontSize: 10 }}>Edit ✎</button>
          )}
        </div>

        {/* Content grid: photo + info | certificate */}
        <div style={{ display: 'grid', gridTemplateColumns: hasCert ? '1fr 200px' : '1fr', gap: 20, alignItems: 'start' }}>

          {/* Left: photo + name + message */}
          <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start' }}>
            ...
            <div style={{ flexShrink: 0 }}>
              {spotlight.photo_url ? (
                <div style={{
                  ...
                  boxShadow: '0 0 20px rgba(201,168,76,.15)',
                  overflow: 'hidden',
                }}>
                  <img
                    src={spotlight.photo_url}
                    alt={spotlight.employee_name}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 2 }}>
                <p style={{ margin: 0, fontSize: 17, fontWeight: 700, color: '#e8e0c8', lineHeight: 1.2, letterSpacing: '-.01em' }}>
                  {spotlight.employee_name}
                </p>
                <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.12em', color: '#c9a84c', textTransform: 'uppercase' }}>
                  Employee of the Month
                </span>
              </div>

              {spotlight.employee_title && (
                <p style={{ margin: '2px 0 10px', fontSize: 11, color: '#5a5a7a', fontWeight: 500 }}>
                  {spotlight.employee_title}
                </p>
              )}
              ...
              {spotlight.message && (
                <div style={{
                  ...
                  <p style={{
                    margin: 0, fontSize: 12.5, color: '#9090b0', lineHeight: 1.7,
                    fontStyle: 'italic',
                    display: '-webkit-box',
                    WebkitLineClamp: 4,
          ...
          {hasCert && (
            <div>
              <p style={{ margin: '0 0 6px', fontSize: 9, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#3a3a5a' }}>
                Certificate
              </p>
              <button
                onClick={() => setCertOpen(true)}
                style={{
                  width: '100%', padding: 0, border: '1px solid #2a2a3e',
                  borderRadius: 8, background: '#0d0d14', cursor: 'pointer',
                  overflow: 'hidden', display: 'block',
                  transition: 'border-color .15s, box-shadow .15s',
                }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(201,168,76,.4)';
                  (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 0 12px rgba(201,168,76,.08)';
                }}
                onMouseLeave={e => {
                ...
                {spotlight.certificate_type === 'pdf' ? (
                  ...
                  <div style={{ height: 130, overflow: 'hidden', position: 'relative' }}>
                    ...
                    <div style={{
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.1em', color: '#fff', textTransform: 'uppercase', background: 'rgba(0,0,0,.5)', padding: '4px 8px', borderRadius: 4 }}>
                        View Full
                      </span>
                    </div>
                  </div>
