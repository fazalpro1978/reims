...
import UnitImportMapper, { type MappedPayload } from './UnitImportMapper';
import UnitImportValidator from './UnitImportValidator';

// ─── Legacy AI Import (image / PDF) — unchanged extraction flow ───────────────

type LegacyStage = 'upload' | 'identifying' | 'verify' | 'validating' | 'validate' | 'importing' | 'done';
...
function LegacyAIImport() {
  const [stage, setStage] = useState<LegacyStage>('upload');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <StageIndicator
          current={order[stage] ?? 0}
          steps={['1. Upload', '2. Verification', '3. Validation', 'Import']}
        />
      </div>
      ...
      {(stage === 'upload' || stage === 'identifying') && (
        <div className="max-w-lg mx-auto space-y-4">
          <div>
            <h2 className="text-lg font-bold text-white">Legacy AI Import</h2>
            <p className="text-sm text-[#555] mt-1">
              For images and PDFs only. Claude reads unstructured layouts automatically — structured CSV/XLSX files should use the Structured Import tab instead.
            </p>
          </div>
            ...
            {stage === 'identifying' ? (
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#444] mt-1">PDF · JPG · PNG · WebP</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-[#555] mt-0.5">Review extracted records. Edit or remove rows before validation.</p>
            </div>
      ...
      {stage === 'done' && result && (
        <ImportDoneScreen result={result} onReset={reset} />
      )}
    </div>
  );
}

// ─── Structured Import (CSV / XLSX) — deterministic Mapping → Validation ──────
...
function StructuredImport() {
  ...
  if (!payload) {
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-end">
          <StageIndicator current={0} steps={['1. Mapping', '2. Validation', 'Import']} />
        </div>
        <UnitImportMapper onMapped={setPayload} />
      </div>
    );
  }

  return <UnitImportValidator payload={payload} onReset={() => setPayload(null)} />;
}
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-[#0f0f0f]'}>
          ...
          <div className="flex items-center gap-1 bg-[#141414] border border-[#222] rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${
                tab === 'structured' ? 'bg-[#c9a84c] text-[#0f0f0f]' : 'text-[#888] hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${
                tab === 'legacy' ? 'bg-[#c9a84c] text-[#0f0f0f]' : 'text-[#888] hover:text-white'
              }`}
            >
              Legacy AI Import (Image/PDF)
            </button>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-screen-xl mx-auto px-5 py-8">
        {tab === 'structured' ? <StructuredImport /> : <LegacyAIImport />}
      </main>
    </div>
  );
}
