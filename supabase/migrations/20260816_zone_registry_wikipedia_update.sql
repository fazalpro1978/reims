-- ─────────────────────────────────────────────────────────────────────────────
-- Zone / District Registry — Wikipedia authoritative update
-- Source: https://en.wikipedia.org/wiki/Zones_of_Qatar (2015 census data)
-- Adds area_km2 and population columns.
-- Corrects district names and municipality assignments that diverged from the
-- official Qatar administrative zone numbering.
...
-- 2. Upsert all zones — Wikipedia authoritative, 2015 census
--    ON CONFLICT DO UPDATE replaces any wrong name/municipality/stats.
...
-- Zone 60 (Al Dafna) is in the registry but not listed in Wikipedia.
-- Retain it; update municipality to consistent naming, leave stats null.
UPDATE public.cr_zone_codes
SET municipality = 'Doha Municipality'
WHERE zone_code = 60 AND district_name = 'Al Dafna';

-- 3. Cascade corrected district names to units.zone so inventory,
--    dashboards, reports, and synergy cards all reflect updated names.
